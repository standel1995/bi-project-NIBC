load('E:/Pelican_Install_Dir/Pelican/ace.product/svc-camel-creationapi/javascript/utility.js');
load('E:/Pelican_Install_Dir/Pelican/ace.product/svc-camel-creationapi/javascript/pelicanxmlutility.js');
load('E:/Pelican_Install_Dir/Pelican/ace.product/svc-camel-creationapi/javascript/messageRepair.js');
...
function checkT2EnrichmentFlag(exchange) {
	...
	var T2EnrichmentFlag = memTblGetTableValue(map, "FLAG-TABLE", "T2Enrichment");
	...
	//if(isPatternPresent(paymentType, "TARGET2")) {
		...
		logger.info("checkT2EnrichmentFlag: PLCN_T2EnrichmentFlag = " + target2MsgType);
	//}
}
...
function setWfHeader(exchange) {
	...
	setHeader(map, "DISPLAY_FLAG_BATCH", "N");
...
function ruleGenerateKbPath(exchange) {
	...
	if(!priorityDate && msgType == "camt.057.001.06") {
	...
	if(receiverPath) {
		...
		logger.trace("ruleGenerateKbPath: receiver from xPath = " + receiver);
		setHeader(map, "PLCN_receiver", receiver);
	}
	...
	/*if(msgType == "camt.057.001.06") {
		...
		if(!sender) {
			senderPath = "NIBCNLNV" + "." + "MESSAGE_PROCESSING.FUNCTIONALITY.DEFAULT_BAH_VALUE.CAMT.057" + "." + "SENDER";
		...
		if(!receiver) {
			receiverPath = "NIBCNLNV" + "." + "MESSAGE_PROCESSING.FUNCTIONALITY.DEFAULT_BAH_VALUE.CAMT.057" + "." + "RECEIVER";
	...
	setHeader(map, "PLCN_INSTITUTIONID", "NIBCNLNV");
	...
	setHeader(map, "PLCN_institutionId", "NIBCNLNV");
	setHeader(map, "PLCNAPI_institutionId", "NIBCNLNV");
...
function drveProductCodeCreation(exchange) {
	...
	logger.info("In drveProductCodeCreation");

	institutionId = getHeader(map, "PLCN_institutionId");
	logger.trace("drveProductCodeCreation: institutionId = " + institutionId);
