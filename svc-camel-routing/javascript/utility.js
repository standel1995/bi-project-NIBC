...
function setCommentsForTransaction(fldCode, errorCode2, map) { //(71, 1560, map)
	...
	logger.trace("setCommentsForTransaction: Invalid Message");
	...
	logger.trace("setCommentsForTransaction: PLCN_txnComments = " + PLCN_txnComments);
	...
	logger.trace('setCommentsForTransaction: finalPLCN_txnComments = ' + finalPLCN_txnComments);
	...
	if(PatternStr){
		if(PatternStr.length > 0) {
			if(str){
				...
				logger.info("isPatternPresent: n = " + n);
...
function getWeekday(dateStr) {
	...
	//logger.trace("getWeekday: year = " + ccyy);   
	//logger.trace("getWeekday: month = " + mm);
	//logger.trace("getWeekday: day = " + dd);
	
	dateStr = [mm, dd, ccyy].join('/');
	//logger.trace("getWeekday: dateStr = " + dateStr);
	
	var date = new Date(dateStr);
	//logger.trace("getWeekday: date = " + date);
