load('E:/Pelican/ace.solution.ppay/svc-camel-cbpr/javascript/pelicanxmlutility.js');
load('E:/Pelican/ace.solution.ppay/svc-camel-cbpr/javascript/utility.js');
load('E:/Pelican/ace.solution.ppay/svc-camel-cbpr/javascript/messageRepair.js');
load('E:/Pelican/ace.solution.ppay/svc-camel-cbpr/javascript/authorizationCheck.js');
...
function dbOperation(exchange) {
	...
	if(msgType === 'pacs.004.001.09'){
		...
		logger.info("populateMetaDataInfoPacs004: msgdbMap = " + msgdbMap);
	}
...
function createHeader(exchange) {
	...
	if(interActFlag == "Y") {
        ...
        var ele1 = doc.getElementsByTagName("Body").item(0).appendChild(newNode);//appending of both

        var newNode2 = doc.importNode(doc2.getFirstChild(), true);
        var ele2 = doc.getElementsByTagName("Body").item(0).appendChild(newNode2);//appending of both
...
function cbprBahValuesMxPacs004(Document, map) {
	...
	hmap.put("from", instgAgt);
	hmap.put("to", instdAgt);
	...
	if(!clrSysMmbIdValue) {
		institutionId = memTblGetTableValue(map, "INST_HIERARCHY", institutionId);
		logger.info("cbprBahValuesMxPacs004: parent institutionId = " + institutionId);
		var clrSysMmbIdPath = institutionId + "." + profile + ".BAH.CBPR_HEADER.CLRSYSMMBID";
		var clrSysMmbIdValue = memTblGetTableValue(map, "INST_PARAM", clrSysMmbIdPath);
		logger.info("cbprBahValuesMxPacs004: clrSysMmbIdValue = " + clrSysMmbIdValue);
	}
...
function cbprBahValuesMxPacs008(Document, map) {
	...
	instgAgt = getValueFromPath(Document, instgAgtPath);
	logger.trace("cbprBahValuesMxPacs008: instgAgt = " + instgAgt);

	instdAgtPath = "/Document/FIToFICstmrCdtTrf/CdtTrfTxInf/InstdAgt/FinInstnId/BICFI";
	instdAgt = getValueFromPath(Document, instdAgtPath);
	logger.trace("cbprBahValuesMxPacs008: instdAgt = " + instdAgt);
	...
	if(!clrSysMmbIdValue) {
		institutionId = memTblGetTableValue(map, "INST_HIERARCHY", institutionId);
		logger.info("cbprBahValuesMxPacs008: parent institutionId = " + institutionId);
		var clrSysMmbIdPath = institutionId + "." + profile + ".BAH.CBPR_HEADER.CLRSYSMMBID";
		var clrSysMmbIdValue = memTblGetTableValue(map, "INST_PARAM", clrSysMmbIdPath);
		logger.info("cbprBahValuesMxPacs008: clrSysMmbIdValue = " + clrSysMmbIdValue);
	}
...
function cbprBahValuesMxPacs009(Document, map) {
	...
	instgAgt = getValueFromPath(Document, instgAgtPath);
	logger.trace("cbprBahValuesMxPacs009: instgAgt = " + instgAgt);

	instdAgtPath = "/Document/FICdtTrf/CdtTrfTxInf/InstdAgt/FinInstnId/BICFI";
	instdAgt = getValueFromPath(Document, instdAgtPath);
	logger.trace("cbprBahValuesMxPacs009: instdAgt = " + instdAgt);
	...
	if(undrlygCstmrCdtTrf) {
		...
		logger.info("cbprBahValuesMxPacs009: messageDefinitionIdentifier = Pacs.009.001.08COVER");
	}else{
		hmap.put("MsgDefIdr", "pacs.009.001.08CORE");
		logger.info("cbprBahValuesMxPacs009: messageDefinitionIdentifier = Pacs.009.001.08CORE");
	}
	...
	if(!clrSysMmbIdValue) {
		institutionId = memTblGetTableValue(map, "INST_HIERARCHY", institutionId);
		logger.info("cbprBahValuesMxPacs009: parent institutionId = " + institutionId);
		var clrSysMmbIdPath = institutionId + "." + profile + ".BAH.CBPR_HEADER.CLRSYSMMBID";
		var clrSysMmbIdValue = memTblGetTableValue(map, "INST_PARAM", clrSysMmbIdPath);
		logger.info("cbprBahValuesMxPacs009: clrSysMmbIdValue = " + clrSysMmbIdValue);
	}
...
function cbprBahValuesMxCamt057(Document, map) {
	...
	logger.trace("target2BahValuesMxPacs009: sender = " + sender);
	logger.trace("target2BahValuesMxPacs009: receiver = " + receiver);
