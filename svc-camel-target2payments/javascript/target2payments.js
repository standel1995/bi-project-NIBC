load('E:/Pelican_Install_Dir/pelican/ace.solution.ppay/svc-camel-target2payments/javascript/pelicanxmlutility.js');
load('E:/Pelican_Install_Dir/pelican/ace.solution.ppay/svc-camel-target2payments/javascript/utility.js'); 
load('E:/Pelican_Install_Dir/pelican/ace.solution.ppay/svc-camel-target2payments/javascript/messageRepair.js');
load('E:/Pelican_Install_Dir/pelican/ace.solution.ppay/svc-camel-target2payments/javascript/authorizationCheck.js');
...
function ruleGenerateKbJs(exchange) {
	...
	if(prevQueueId == "MXMSGAH" && custom13 == "AUTH=Y") {
		setHeader(map, "PLCN_custom13", "AUTH=D");
		logger.info("ruleGenerateKbJs: PLCN_custom13 = " + getHeader(map, "PLCN_custom13"));
	}else if(prevQueueId == "MXMSGAH" || custom13 == "AUTH=N") {
		setHeader(map, "PLCN_callSchCheck", true);
		logger.info("ruleGenerateKbJs: PLCN_callSchCheck = " + getHeader(map, "PLCN_callSchCheck"));		
	}else {
		setHeader(map, "PLCN_callAuthCheck", true);
		logger.info("ruleGenerateKbJs: PLCN_callAuthCheck = " + getHeader(map, "PLCN_callAuthCheck"));		
	}
...
function dbOperation(exchange) {
	...
	msgdbMap = new HashMap(); // getHeader(map, "ACEQ_WRITE_MSGDB"); //
	//logger.info("dbOperation: msgdbMap = " + msgdbMap);
	...
	logger.info("dbOperation: msgBlock2 = " + msgBlock2);
	...
	logger.info("dbOperation: validMessage = " + validMessage);

	var responseCdsDoc = getHeader(map, "ACEDB_responseCdsDoc");
	logger.info("dbOperation: typeof responseCdsDoc = " + typeof responseCdsDoc);
	...
	logger.info("dbOperation: PLCN_queue = " + getHeader(map, "PLCN_queue"));
	logger.info("dbOperation: PLCN_QUEUE = " + getHeader(map, "PLCN_QUEUE"));
...
function drveNibcProductCode(exchange) {
	...
	if(isPatternPresent(drveProductCodeFlag, msgType)) {
		if(mode == "MANUAL" || mode == "UPLOAD") {
			...
			logger.trace("drveNibcProductCode: productCode = " + productCode);
		}
...
function setValidationHeaderTarget2(exchange) {
	var inMsg = exchange.getIn();
	var map = inMsg.getHeaders();
	...
	setHeader(map, "PaymentType", "TARGET2" + msgType);
	setHeader(map, "CamelHttpMethod", "POST");
	//setHeader(map, "PLCN_Target2Flag", true);
	logger.info("setValidationHeaderTarget2: PaymentType = " + getHeader(map, "PaymentType"));
	logger.info("setValidationHeaderTarget2: CamelHttpMethod = " + getHeader(map, "CamelHttpMethod"));
}
...
function createHeader(exchange) {
	...
	var bahFlag = memTblGetTableValue(map, "FLAG-TABLE", "TARGET2_BAH");
	logger.info("createHeader: bahFlag = " + bahFlag);
	...
	var interActFlag = memTblGetTableValue(map, "FLAG-TABLE", "TARGET2_INTERACT");
	...
	if(interActFlag == "Y") {
 	...
 	/*if(msgDirection == "I") {
		var lauFlag = memTblGetTableValue(map, "FLAG-TABLE", "OB_TARGET2_LAU");
		logger.info("createHeader: lauFlag = " + lauFlag);
		lauFlag = lauFlag.trim();
 	}else {
		var lauFlag = memTblGetTableValue(map, "FLAG-TABLE", "IB_TARGET2_LAU");
		logger.info("createHeader: lauFlag = " + lauFlag);
		lauFlag = lauFlag.trim();
 	}*/
...
function generateInterAct(exchange) {
	...
	if(interActMap) {
		...
		logger.trace("generateInterAct: receiver = " + receiver); //APKEFIHHXXX
		...
		if(!paramvalue)	{
			logger.info("generateInterAct: receiver = DEFAULT_T2");
			paramvalue = memTblGetTableValue(map, interActMap, "DEFAULT_T2");
	...
	if(firstPath) {
		...
		if(msgType == "pacs.008.001.08") {
			mType = "T2pacs.008"; //PLCNGBWB.OPERATIONAL_CONF.BANK_PROFILE_01.INTERACT.T2pacs.008.MESSAGE_IDENTIFIER
...
function generateBAH(exchange) {
	...
	if(msgType == 'pacs.004.001.09') {
		target2BahValuesMxPacs004(Document, map);
	}
...
function validateLAU(exchange) {
	...
	logger.trace("validateLAU: typeof msgBlock1 = " + typeof msgBlock1);
...
*/
function target2BahValuesMxPacs004(Document, map) {
	var instdAgt;
	var instgAgt;
	var msgId;
	var msgClassType;
	...
	if(msgId == "NONREF") {
		...
		logger.info("target2BahValuesMxPacs004: businessMessageIdentifier = " + messageNo);
	}

	if(!(msgId == "NONREF")){
		hmap.put("BizMsgIdr", msgId);
		logger.info("target2BahValuesMxPacs004: businessMessageIdentifier = " + msgId);
	}

	var date = getDateBAH();
	var time = localTime();
	var CreDt = date + "T" + time +"Z";
	logger.info("target2BahValuesMxPacs004: CreDt = " + CreDt);
	...
	var institutionId = getHeader(map, "PLCN_institutionId");
	logger.info("target2BahValuesMxPacs004: institutionId = " + institutionId);

	var profile = getProfile(Document, map);
	logger.info("target2BahValuesMxPacs004: profile = " + profile);
	
	//var clrSysMmbIdPath = institutionId + ".OPERATIONAL_CONF.BANK_PROFILE_01.BAH.RTGS_HEADER.CLRSYSMMBID";
	var clrSysMmbIdPath = institutionId + "." + profile + ".BAH.RTGS_HEADER.CLRSYSMMBID";
	logger.info("target2BahValuesMxPacs004: clrSysMmbIdPath = " + clrSysMmbIdPath);
	var clrSysMmbIdValue = memTblGetTableValue(map, "INST_PARAM", clrSysMmbIdPath);
	logger.info("target2BahValuesMxPacs004: clrSysMmbIdValue = " + clrSysMmbIdValue);

	if(!clrSysMmbIdValue) {	
		institutionId = memTblGetTableValue(map, "INST_HIERARCHY", institutionId);
		logger.info("target2BahValuesMxPacs004: parent institutionId = " + institutionId);
		var clrSysMmbIdPath = institutionId + "." + profile + ".BAH.RTGS_HEADER.CLRSYSMMBID";
		logger.info("target2BahValuesMxPacs004: clrSysMmbIdPath = " + clrSysMmbIdPath);
		var clrSysMmbIdValue = memTblGetTableValue(map, "INST_PARAM", clrSysMmbIdPath);
		logger.info("target2BahValuesMxPacs004: clrSysMmbIdValue = " + clrSysMmbIdValue);
	...
	var bah = appHeaderHandler.createBahHeader(hmap, bahFrMsg, "head.001.001.01");
	logger.trace("target2BahValuesMxPacs004: bah = " + bah);
...
function target2BahValuesMxPacs008(Document, map) {
	...
	var from = getHeader(map, "PLCN_sender");
	logger.trace("target2BahValuesMxPacs008: sender = " + from);	
	var to = getHeader(map, "PLCN_receiver");
	logger.trace("target2BahValuesMxPacs008: receiver = " + to);	

	if(!from) {
		instgAgtPath = "/Document/FIToFICstmrCdtTrf/CdtTrfTxInf/InstgAgt/FinInstnId/BICFI";
		from = getValueFromPath(Document, instgAgtPath);
		logger.trace("target2BahValuesMxPacs008: instgAgt = " + from);
	}

	if(!to) {
		instdAgtPath = "/Document/FIToFICstmrCdtTrf/CdtTrfTxInf/InstdAgt/FinInstnId/BICFI";
		to = getValueFromPath(Document, instdAgtPath);
		logger.trace("target2BahValuesMxPacs008: instdAgt = " + to);
	}

	msgIdPath = "/Document/FIToFICstmrCdtTrf/GrpHdr/MsgId";
	msgId = getValueFromPath(Document, msgIdPath);
	logger.info("target2BahValuesMxPacs008: msgId = " + msgId);
	...
	if(msgId == "NONREF") {
		hmap.put("BizMsgIdr", messageNo);
		logger.info("target2BahValuesMxPacs008: businessMessageIdentifier = " + messageNo);
	}

	if(!(msgId == "NONREF")){
		hmap.put("BizMsgIdr", msgId);
		logger.info("target2BahValuesMxPacs008: businessMessageIdentifier = " + msgId);
	}

	var date = getDateBAH();
	var time = localTime();
	var CreDt = date + "T" + time +"Z";
	logger.info("target2BahValuesMxPacs008: CreDt = " + CreDt);
	...
	var institutionId = getHeader(map, "PLCN_institutionId");
	logger.info("target2BahValuesMxPacs008: institutionId = " + institutionId);

	var profile = getProfile(Document, map);
	logger.info("target2BahValuesMxPacs008: profile = " + profile);
	
	//var clrSysMmbIdPath = institutionId + ".OPERATIONAL_CONF.BANK_PROFILE_01.BAH.RTGS_HEADER.CLRSYSMMBID";
	var clrSysMmbIdPath = institutionId + "." + profile + ".BAH.RTGS_HEADER.CLRSYSMMBID";
	logger.info("target2BahValuesMxPacs008: clrSysMmbIdPath = " + clrSysMmbIdPath);
	var clrSysMmbIdValue = memTblGetTableValue(map, "INST_PARAM", clrSysMmbIdPath);
	logger.info("target2BahValuesMxPacs008: clrSysMmbIdValue = " + clrSysMmbIdValue);

	if(!clrSysMmbIdValue) {
		institutionId = memTblGetTableValue(map, "INST_HIERARCHY", institutionId);
		logger.info("target2BahValuesMxPacs008: parent institutionId = " + institutionId);
		var clrSysMmbIdPath = institutionId + "." + profile + ".BAH.RTGS_HEADER.CLRSYSMMBID";
		logger.info("target2BahValuesMxPacs008: clrSysMmbIdPath = " + clrSysMmbIdPath);
		var clrSysMmbIdValue = memTblGetTableValue(map, "INST_PARAM", clrSysMmbIdPath);
		logger.info("target2BahValuesMxPacs008: clrSysMmbIdValue = " + clrSysMmbIdValue);			
	...
	var bah = appHeaderHandler.createBahHeader(hmap, bahFrMsg, "head.001.001.01");
	logger.trace("target2BahValuesMxPacs008: bah = " + bah);
	setHeader(map, "PLCN_bah", bah);
}
...
function target2BahValuesMxPacs009(Document, map) {
	...
	var from = getHeader(map, "PLCN_sender");
	logger.trace("target2BahValuesMxPacs009: sender = " + from);	
	var to = getHeader(map, "PLCN_receiver");
	logger.trace("target2BahValuesMxPacs009: receiver = " + to);	

	if(!from) {
		instgAgtPath = "/Document/FICdtTrf/CdtTrfTxInf/InstgAgt/FinInstnId/BICFI";
		from = getValueFromPath(Document, instgAgtPath);
		logger.trace("target2BahValuesMxPacs009: instgAgt = " + from);
	}

	if(!to) {
		instdAgtPath = "/Document/FICdtTrf/CdtTrfTxInf/InstdAgt/FinInstnId/BICFI";
		to = getValueFromPath(Document, instdAgtPath);
		logger.trace("target2BahValuesMxPacs009: instdAgt = " + to);
	}

	msgIdPath = "/Document/FICdtTrf/GrpHdr/MsgId";
	msgId = getValueFromPath(Document, msgIdPath);
	logger.info("target2BahValuesMxPacs009: msgId = " + msgId);

	messageno = getHeader(map, "PLCN_messageNo");

	if(msgId == "NONREF"){
		hmap.put("BizMsgIdr", messageno);
		logger.info("target2BahValuesMxPacs009: businessMessageIdentifier = " + messageno);
	}

	if(!(msgId == "NONREF")){
		hmap.put("BizMsgIdr", msgId);
		logger.info("target2BahValuesMxPacs009: businessMessageIdentifier = " + msgId);
	}
	...
	if(undrlygCstmrCdtTrf) {
		...
		logger.info("target2BahValuesMxPacs009: messageDefinitionIdentifier = Pacs.009.001.08COVER");
	}else{
		hmap.put("MsgDefIdr", "pacs.009.001.08CORE");
		logger.info("target2BahValuesMxPacs009: messageDefinitionIdentifier = Pacs.009.001.08CORE");
	}
	...
	var institutionId = getHeader(map, "PLCN_institutionId");
	logger.info("target2BahValuesMxPacs009: institutionId = " + institutionId);

	var profile = getProfile(Document, map);
	logger.info("target2BahValuesMxPacs009: profile = " + profile);
	
	//var clrSysMmbIdPath = institutionId + ".OPERATIONAL_CONF.BANK_PROFILE_01.BAH.RTGS_HEADER.CLRSYSMMBID";
	var clrSysMmbIdPath = institutionId + "." + profile + ".BAH.RTGS_HEADER.CLRSYSMMBID";
	logger.info("target2BahValuesMxPacs009: clrSysMmbIdPath = " + clrSysMmbIdPath);
	var clrSysMmbIdValue = memTblGetTableValue(map, "INST_PARAM", clrSysMmbIdPath);
	logger.info("target2BahValuesMxPacs009: clrSysMmbIdValue = " + clrSysMmbIdValue);

	if(!clrSysMmbIdValue) {
		institutionId = memTblGetTableValue(map, "INST_HIERARCHY", institutionId);
		logger.info("target2BahValuesMxPacs009: parent institutionId = " + institutionId);
		var clrSysMmbIdPath = institutionId + "." + profile + ".BAH.RTGS_HEADER.CLRSYSMMBID";
		logger.info("target2BahValuesMxPacs009: clrSysMmbIdPath = " + clrSysMmbIdPath);
		var clrSysMmbIdValue = memTblGetTableValue(map, "INST_PARAM", clrSysMmbIdPath);
		logger.info("target2BahValuesMxPacs009: clrSysMmbIdValue = " + clrSysMmbIdValue);
	...
	var bah = appHeaderHandler.createBahHeader(hmap, bahFrMsg, "head.001.001.01");
	logger.trace("target2BahValuesMxPacs009: bah = " + bah);
	setHeader(map, "PLCN_bah", bah);
}
...
function getProfile(Document, map) {
	...
	if(interActMap) {
		var receiver = getHeader(map, "PLCN_receiver"); 
		logger.info("getProfile: receiver = " + receiver); //APKEFIHHXXX

		interActMap = interActMap + "_MAP";
		
		var paramvalue = memTblGetTableValue(map, interActMap, receiver); //"SWIFT_INTERACT_PROFILE_01"
		logger.info("getProfile: paramvalue = " + paramvalue); //SWIFT_INTERACT_PROFILE_01

		if(!paramvalue)	{
			logger.info("getProfile: receiver = DEFAULT_T2");
			paramvalue = memTblGetTableValue(map, interActMap, "DEFAULT_T2");
...
function setExceptionViolation(exchange) {
	...
	if(httpException != "") {
		...
		if(httpException == "true") {
			setCommentsForTransaction("00", "7389", map);
		}
	}		
