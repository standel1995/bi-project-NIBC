...
function townNameAndCountryRuleSepaPacs8(exchange) {  //DONE 
	...
	if(!cdtrAddrLine) {
		setHeader(map, "PLCN_cdtrAddrLine", false);
		logger.info("townNameAndCountryRuleSepaPacs8: PLCN_cdtrAddrLine = false");
	}else {
		setHeader(map, "PLCN_cdtrAddrLine", true);
		logger.info("townNameAndCountryRuleSepaPacs8: PLCN_cdtrAddrLine = true");
	}

	if(!dbtrAddrLine) {
		setHeader(map, "PLCN_dbtrAddrLine", false);
		logger.info("townNameAndCountryRuleSepaPacs8: PLCN_dbtrAddrLine = false");
	}else {
		setHeader(map, "PLCN_dbtrAddrLine", true);
		logger.info("townNameAndCountryRuleSepaPacs8: PLCN_dbtrAddrLine = true");
	}
		...
		if(isPatternPresent(Document1, "<Cdtr>")){
			if(cdtrPstlAdr){
				if(cdtrAddrLine && (cdtrDept||cdtrSubDept||cdtrStrtNm||cdtrBldgNb||cdtrBldgNm||cdtrFlr||cdtrPstBx||cdtrRoom||cdtrPstCd||cdtrTwnNm||cdtrTwnLctnNm ||cdtrDstrctNm||cdtrCtrySubDvsn)){
					...
					setHeader(map, "PLCN_cdtrPstlAdrChecked", true);
					return retVal;
				}
			}
		}
		...
		if(isPatternPresent(Document1, "<Dbtr>")){
			if(dbtrPstlAdr){
				if(dbtrAddrLine && (dbtrDept||dbtrSubDept||dbtrStrNm||dbtrBldgNb||dbtrBldgNm||dbtrFlr||dbtrPstBx||dbtrRoom||dbtrPstCd||dbtrTwnLctnNm||dbtrDstrctNm||dbtrCtrySubDvsn||dbtrTwnNm)){
					...
					setHeader(map, "PLCN_dbtrPstlAdrChecked", true);
					return retVal;
				}
			}
		}
...
function hybridAddressRuleSepaPacs8(exchange){ 
	...
	var cdtrPstlAdrChecked = getHeader(map, "PLCN_cdtrPstlAdrChecked");
	logger.info("hybridAddressRuleSepaPacs8: cdtrPstlAdrChecked = " + cdtrPstlAdrChecked);
	logger.info("hybridAddressRuleSepaPacs8: typeof cdtrPstlAdrChecked = " + typeof cdtrPstlAdrChecked);

	var dbtrPstlAdrChecked = getHeader(map, "PLCN_dbtrPstlAdrChecked");
	logger.info("hybridAddressRuleSepaPacs8: dbtrPstlAdrChecked = " + dbtrPstlAdrChecked);
	logger.info("hybridAddressRuleSepaPacs8: typeof dbtrPstlAdrChecked = " + typeof dbtrPstlAdrChecked);

	if(sysDate >= Date1){
		logger.info("hybridAddressRuleSepaPacs8: hybridAddressRuleSepaPacs8 will be applied");

		if(cdtrPstlAdrChecked != true) {
			//CREDITOR
			logger.info("hybridAddressRuleSepaPacs8: hybridAddressRuleSepaPacs8 will be applied for Cdtr");
			var cdtrPstlAdr =  isXmlNodePresent(Document, "CdtTrfTxInf", "Cdtr", "<PstlAdr>");
		...
		if(dbtrPstlAdrChecked != true) {
			//DEBTOR
			logger.info("hybridAddressRuleSepaPacs8: hybridAddressRuleSepaPacs8 will be applied for Dbtr");
			var dbtrPstlAdr =  isXmlNodePresent(Document, "CdtTrfTxInf", "Dbtr", "<PstlAdr>");
