...
function b2bIntrBnkSttltDate(Document, map) {
	...
	if(ttlIntrBkSttlmAmt){
		if(!grpHdrIntrBkSttlmDt && !cdtrTrfinfintrBkStlDt){
				...
				retVal = setCommentsForTransaction("324", "5773", map);	
				return retVal;	 
		}
	}
	return retVal;
...
function grpHdr_CdtTrfTxInf_FldCompRulePacs008(Document, map) {
	...
	logger.info("grpHdr_CdtTrfTxInf_FldCompRulePacs008 = " + pacs08ValdFlagMx);

	retVal = 0;
	
	messageDirection = getHeader(map, "PLCN_msgDirection");
	logger.info("grpHdr_CdtTrfTxInf_FldCompRulePacs008: messageDirection = " + messageDirection);

	var xsdCheckFlag = getHeader(map, "PLCN_xsdCheckFlag");
	logger.info("grpHdr_CdtTrfTxInf_FldCompRulePacs008: xsdCheckFlag = " + xsdCheckFlag);
	...
	if(pacs08ValdFlagMx == 'ERROR') {
		if(grpHdrInstgAgt && cdtrTrfinInstgAgt){
			...
			retVal = setCommentsForTransaction("523", "5702", map);
			return retVal;
		}
		if(!grpHdrInstgAgt && !cdtrTrfinInstgAgt){
			if(xsdCheckFlag == "EQUENS" && messageDirection == "O") {
				// setHeader(map, "PLCN_validMessage",false);
				// logger.info("grpHdr_CdtTrfTxInf_FldCompRulePacs008 Error block: InstgAgt must be present in grp header or in transaction.");
				// retVal = setCommentsForTransaction("523", "8020", map);
				return retVal;
			}else {
				setHeader(map, "PLCN_validMessage",false);
				logger.info("grpHdr_CdtTrfTxInf_FldCompRulePacs008 Error block: InstgAgt must be present in grp header or in transaction.");
		...
		if(grpHdrInstdAgt && cdtrTrfinInstdAgt){
			...
			retVal = setCommentsForTransaction("536", "5701", map);
		}
		if(!grpHdrInstdAgt && !cdtrTrfinInstdAgt){
			if(xsdCheckFlag == "EPC" && messageDirection == "I") {
				// setHeader(map, "PLCN_validMessage",false);
				// logger.info("grpHdr_CdtTrfTxInf_FldCompRulePacs008 Error block: InstdAgt must be present in grp header or in transaction.");
				// retVal = setCommentsForTransaction("536", "8019", map);
				return retVal;
			}else {
				setHeader(map, "PLCN_validMessage",false);
				logger.info("grpHdr_CdtTrfTxInf_FldCompRulePacs008 Error block: InstdAgt must be present in grp header or in transaction.");
		...
		if(grpHdrPmtTpInf && cdtrTrfinPmtTpInf){
			...
			retVal = setCommentsForTransaction("124", "5703", map);
			return retVal;
		}
		if(!grpHdrPmtTpInf && !cdtrTrfinPmtTpInf){
			setHeader(map, "PLCN_validMessage",false);
			logger.info("grpHdr_CdtTrfTxInf_FldCompRulePacs008 Error block: payment type info must be present in grp header or in transaction.");
			retVal = setCommentsForTransaction("124", "8021", map);
			return retVal;
		}
	}
	...
	if(pacs08ValdFlagMx == 'WARNING') {
		if(grpHdrInstgAgt && cdtrTrfinInstgAgt){
			setHeader(map, "PLCN_validMessage",false);
			logger.info("grpHdr_CdtTrfTxInf_FldCompRulePacs008: If GrpHdr/InstgAgt is present, then CdtTrfTxInf/InstgAgt is not allowed.");
			retVal = setCommentsForTransaction("523", "7040", map);
			return retVal;
		}
		if(!grpHdrInstgAgt && !cdtrTrfinInstgAgt){
			setHeader(map, "PLCN_validMessage",false);
			logger.info("grpHdr_CdtTrfTxInf_FldCompRulePacs008 Warning block: InstgAgt must be present in grp header or in transaction.");
			retVal = setCommentsForTransaction("523", "8020", map);
			return retVal;
		}
		if(grpHdrInstdAgt && cdtrTrfinInstdAgt){
			setHeader(map, "PLCN_validMessage",false);
			logger.info("grpHdr_CdtTrfTxInf_FldCompRulePacs008: If GrpHdr/InstdAgt is present, then CdtTrfTxInf/InstdAgt is not allowed.");
			retVal = setCommentsForTransaction("536", "7039", map);
			return retVal;
		}
		if(!grpHdrInstdAgt && !cdtrTrfinInstdAgt){
			setHeader(map, "PLCN_validMessage",false);
			logger.info("grpHdr_CdtTrfTxInf_FldCompRulePacs008 Warning block: InstdAgt must be present in grp header or in transaction.");
			retVal = setCommentsForTransaction("536", "8019", map);
			return retVal;
		}
		if(grpHdrPmtTpInf && cdtrTrfinPmtTpInf){
		...
		if(!grpHdrPmtTpInf && !cdtrTrfinPmtTpInf){
			setHeader(map, "PLCN_validMessage",false);
			logger.info("grpHdr_CdtTrfTxInf_FldCompRulePacs008 Warning block: payment type info must be present in grp header or in transaction.");
			retVal = setCommentsForTransaction("124", "8021", map);
			return retVal;
		}
	}
...
function b2bChargesAmountCurrency(Document, map) {  
	...
	if(chrgInf){
		if(intrBkSttlmtCcy != chrgsAmtCcy){
			...
			setHeader(map, "PLCN_validMessage", false);
			retVal = setCommentsForTransaction("349", "7906", map);
			return retVal;			
		}
	}
	return retVal;
...
function b2bIntrBnkSttltDateRulePacs004(Document, map) {
	...
	if(grpHdrIntrBkSttlmDt && txInfintrBkStlDt){
		setHeader(map, "PLCN_validMessage",false);
		logger.info("b2bIntrBnkSttltDateRulePacs004: If GrpHdr/IntrBkSttlmDt is present, then TxInf/IntrBkSttlmDt is not allowed.");
		retVal = setCommentsForTransaction("181", "7131", map);
		return retVal;
	}

	if(!grpHdrIntrBkSttlmDt && !txInfintrBkStlDt){
		setHeader(map, "PLCN_validMessage",false);
		logger.info("b2bIntrBnkSttltDateRulePacs004: If GrpHdr/IntrBkSttlmDt is Not present, then TxInf/IntrBkSttlmDt must be present..");
		retVal = setCommentsForTransaction("181", "7043", map);
		return retVal;
	}
	...
	if(ttlIntrBkSttlmAmt){
		if(!grpHdrIntrBkSttlmDt && !txInfintrBkStlDt){
				setHeader(map, "PLCN_validMessage",false);
				logger.info("b2bIntrBnkSttltDateRulePacs004: If TtlIntrBkSttlmAmt is present, then IntrBkSttlmDt must be present.");
				retVal = setCommentsForTransaction("181", "5773", map); 	
				return retVal;	 
		}
	}
	return retVal;
...
function genericMustPresentRulePacs004(Document, map) {
	...
	if(instgRmbrsmntAgtAcct){
		if(!instgRmbrsmntAgt){
			...
			retVal = setCommentsForTransaction("153", "7834", map);		
			return retVal;	
		}
	}
	...
	if(instdRmbrsmntAgtAcct){
		if(!instdRmbrsmntAgt){
			...
			retVal = setCommentsForTransaction("156", "7835", map);		
			return retVal;	
		}
	}
	...
	if(thrdRmbrsmntAgtAcct){
		if(!thrdRmbrsmntAgt){
			...
			retVal = setCommentsForTransaction("159", "7833", map);		
			return retVal;	
		}
	}
	...
	if(thrdRmbrsmntAgt){
		if(!instgRmbrsmntAgt && !instdRmbrsmntAgt){
			...
			retVal = setCommentsForTransaction("127", "7828", map);		
			return retVal;	
		}
	}
	return retVal;
...
function grpHdrsttlmtMtdRulePacs004(Document, map) {
	...
	if(sttlmtMtd1 == 'COVE'){
			setHeader(map, "PLCN_validMessage",false);
			logger.info("grpHdrsttlmtMtdRulePacs004: SettlementMethod must be different from COVE");
			retVal = setCommentsForTransaction("128", "7132", map);
			return retVal;			
		}
		return retVal;
}
	...
	var numOfTrans = nbOfTxsTag.item(0);
		...
		if (grpRtr == false){
			if(numOfTrans != count)
			{
				...
				retVal = setCommentsForTransaction("124", "7133", map);
				return retVal;
			}
		...
		if (grpRtr == true){
			if(numOfTrans != count)
			{
				...
				retVal = setCommentsForTransaction("124", "7134", map);
				return retVal;
			}
	}
	return retVal;
...
function genericNotAllowedRulePacs004(Document, map) {
	...
	if(grpHdrInstdAgt)
	{
		if(txInfInstdAgt) {
			...
			retVal = setCommentsForTransaction("244", "7039", map);
			return retVal;			
		}
	}
	...
	if(grpHdrInstgAgt)
	{
		if(txInfInstgAgt) {
			...
			retVal = setCommentsForTransaction("231", "7040", map);
			return retVal;			
		}
	}
	return retVal;
...
function groupReturnAndReturnReasonRulePacs004(Document, map) {
	...
	if(grpRtr == true)
	{
		...
		if(res1 != true )
	    {
			...
			retVal = setCommentsForTransaction("164", "5721", map);
			return retVal;
		}
	}
	return retVal;
...
function groupHeaderGroupReturnFalseRulePacs004(Document, map){
	...
	if( grpHdrGrpRtrVal == false)
	{
		...
		if(res1 != true )
		{
			...
			retVal = setCommentsForTransaction("165","5720", map);
			return retVal;
		}
	}
	return retVal;
...
function grpHdr_CdtTrfTxInf_FldCompRulePacs004(Document, map) {
	...
	logger.info("grpHdr_CdtTrfTxInf_FldCompRulePacs004: grpHdrInstgAgt " + grpHdrInstgAgt);

	cdtrTrfinInstgAgt = isXmlNodePresent(Document,"PmtRtr", "TxInf", "<InstgAgt>");
	logger.info("grpHdr_CdtTrfTxInf_FldCompRulePacs004: cdtrTrfinInstgAgt =" + cdtrTrfinInstgAgt);
	
	//InstructedAgent
	grpHdrInstdAgt = isXmlNodePresent(Document,"PmtRtr", "GrpHdr", "<InstdAgt>");
	logger.info("grpHdr_CdtTrfTxInf_FldCompRulePacs004: grpHdrInstdAgt " + grpHdrInstdAgt);

	cdtrTrfinInstdAgt = isXmlNodePresent(Document,"PmtRtr", "TxInf", "<InstdAgt>");
	logger.info("grpHdr_CdtTrfTxInf_FldCompRulePacs004: cdtrTrfinInstdAgt =" + cdtrTrfinInstdAgt);

	if(grpHdrInstgAgt && cdtrTrfinInstgAgt){
		setHeader(map, "PLCN_validMessage",false);
		logger.info("grpHdr_CdtTrfTxInf_FldCompRulePacs004: If GrpHdr/InstgAgt is present, then TxInf/InstgAgt is not allowed.");
		retVal = setCommentsForTransaction("231", "7039", map);
		return retVal;
	}

	if(grpHdrInstdAgt && cdtrTrfinInstdAgt){
		setHeader(map, "PLCN_validMessage",false);
		logger.info("grpHdr_CdtTrfTxInf_FldCompRulePacs004: If GrpHdr/InstdAgt is present, then TxInf/InstdAgt is not allowed.");
		retVal = setCommentsForTransaction("244", "7040", map);
		return retVal;
	}
...
function orgnlGrpHdrInfoRulePacs004(Document,map) {
	var orgnlGrpInfGrpHdr;
	var orgnlGrpInfTxnInf;
	
	retVal = 0;
	logger.info("In orgnlGrpHdrInfoRulePacs004");
	
	orgnlGrpInfGrpHdr = isXmlNodePresent4(Document, "PmtRtr", "OrgnlGrpInf");
	logger.info("orgnlGrpHdrInfoRulePacs004: orgnlGrpInfGrpHdr = " + orgnlGrpInfGrpHdr);

	orgnlGrpInfTxnInf = isXmlNodePresent4(Document, "TxInf", "OrgnlGrpInf");
	logger.info("orgnlGrpHdrInfoRulePacs004: orgnlGrpInfTxnInf = " + orgnlGrpInfTxnInf);	
	
	if(orgnlGrpInfTxnInf == true && orgnlGrpInfGrpHdr == true){
			setHeader(map, "PLCN_validMessage",false);
			logger.info("orgnlGrpHdrInfoRulePacs004: Original Group Information present in both sequences");
			retVal = setCommentsForTransaction("168", "7054", map);
			return retVal;			
		}
		return retVal;
...
function reimbursementAgentRuleCamt056(Document, map) {
	...
	if(thrdRmbrsmntAgtCheck){
		if(!instgRmbrsmntAgtCheck || !instdRmbrsmntAgtCheck){
			setHeader(map, "PLCN_validMessage",false);
			retVal = setCommentsForTransaction("00", "7828", map); //FldTag not present
			return retVal;
		}	
	}
	...
	if(thrdRmbrsmntAgtAcctCheck){
		if(!thrdRmbrsmntAgtCheck){
			setHeader(map, "PLCN_validMessage",false);
			retVal = setCommentsForTransaction("00", "7833", map); //FldTag not present
			return retVal;
		}	
	}
	...
	if(instgRmbrsmntAgtAcctCheck){
		if(!instgRmbrsmntAgtCheck){
			setHeader(map, "PLCN_validMessage",false);
			retVal = setCommentsForTransaction("00", "7834", map); //FldTag not present
			return retVal;
		}	
	}
	...
	if(instdRmbrsmntAgtAcctCheck){
		if(!instdRmbrsmntAgtCheck){
			setHeader(map, "PLCN_validMessage",false);
			retVal = setCommentsForTransaction("00", "7835", map); //FldTag not present
			return retVal;
		}	
	}
	return retVal;
...
function grpCxlAndRsnRuleCamt056(Document, map) {
	...
	if(grpCxlValue == "true"){
		if(rsnValue){
			setHeader(map, "PLCN_validMessage",false);
			retVal = setCommentsForTransaction("176", "7141", map);
			logger.info("grpCxlAndRsnRuleCamt056: If OrgnlGrpInfAndCxl/GroupCancellation is true, then OrgnlGrpInfAndCxl/CxlRsnInf/Rsn must present.");
			return retVal;
		}
	}
	...
	if(grpCxlValue == "false"){
		if(!txInfValue){
			setHeader(map, "PLCN_validMessage",false);
			retVal = setCommentsForTransaction("155", "7142", map);
			logger.info("grpCxlAndRsnRuleCamt056: If OrgnlGrpInfAndCxl/GroupCancellation is false, then TxInf must be present.");
			return retVal;
		}
	}
	...
	if(grpCxlValue == "true"){
		if(txInfValue){
			setHeader(map, "PLCN_validMessage",false);
			retVal = setCommentsForTransaction("155", "7143", map);
			logger.info("grpCxlAndRsnRuleCamt056: If OrgnlGrpInfAndCxl/GroupCancellation is true, then TxInf is not allowed");
			return retVal;
		}
	}
	return retVal;
...
/* function messageOrGroupCaseRuleCamt056(Document, map) {
	...
	if(caseValue && orgnlGrpInfAndCxlCaseValue && txInfCaseValue){
		setHeader(map, "PLCN_validMessage",false);
		retVal = setCommentsForTransaction("155", "1121", map);
		logger.info("messageOrGroupCaseRuleCamt056: Case may be present at either Case, OrgnlGrpInfAndCxl, OrgnlPmtInfAndCxl or TxInf");
		return retVal;
	}
	
	if(!caseValue && !orgnlGrpInfAndCxlCaseValue && !txInfCaseValue){
		setHeader(map, "PLCN_validMessage",false);
		retVal = setCommentsForTransaction("155", "1131", map);
		logger.info("messageOrGroupCaseRuleCamt056: Case may be present at either Case, OrgnlGrpInfAndCxl, OrgnlPmtInfAndCxl or TxInf");
		return retVal;
	}
	return retVal;
...
function amendmentIndicatorRuleCamt029(Document,map) {
	...
	if(amdmntInd == "TRUE"){
		...
		if(!amdmntInfDtls){
			...
			retVal = setCommentsForTransaction("00", "7936", map); //FldTag not present
	...
	if(amdmntInd == "FALSE"){
		...
		if(amdmntInfDtls){
			...
			retVal = setCommentsForTransaction("00", "7937", map); //FldTag not present
			return retVal;
		}
	}
	return retVal;
...
function genericMustPresentRuleCamt029(Document, map) {
	...
	if(grpCxlStsVal == "PACR"){
		if(!nbOfTxsPerCxlSts){
			setHeader(map, "PLCN_validMessage",false);
			logger.info("genericMustPresentRuleCamt029: OrgnlGrpInfAndSts/NbOfTxsPerCxlSts should only be present if GrpCxlSts equals PACR.");
			retVal = setCommentsForTransaction("00", "7144", map);//FldTag not present		
			return retVal;	
		}
	}
	...
	if(confVal == "PECR" || confVal == "RJCR"){
		if(!cxlDtls){
			setHeader(map, "PLCN_validMessage",false);
			logger.info("genericMustPresentRuleCamt029: If Sts/Sts is present and equal to PECR or RJCR then cxlDtls must be present.");
			retVal = setCommentsForTransaction("00", "5957", map);	//FldTag not present
			return retVal;	
		}
	}
	return retVal;
	...
	if(grpCxlStsVal != null){
		if(grpCxlStsVal != "RJCR" || grpCxlStsVal != "PNCR"){
			...
			if(addtlInf) {
				...
				retVal = setCommentsForTransaction("00", "5958", map);
				return retVal;			
			}
		}
	}
	...
	if(pmtInfCxlStsVal != null){
		if(pmtInfCxlStsVal != "RJCR" || pmtInfCxlStsVal != "PNCR"){
			...
			if(addtlInf) {
				...
				retVal = setCommentsForTransaction("00", "5959", map);
				return retVal;			
			}
		}
	}
...
function settlementMethodRuleCamt029(Document, map) {
	...
	if(sttlmtMtd == 'INDA' || sttlmtMtd == 'INGA'){
		if(instgRmbrsmntAgt || instdRmbrsmntAgt || thrdRmbrsmntAgt || instdRmbrsmntAgtAcct || instgRmbrsmntAgtAcct || thrdRmbrsmntAgtAcct || clrSys){
			...
			retVal = setCommentsForTransaction("00", "7829", map);	//FldTag not present
	...
	if(sttlmtMtd == 'COVE'){
		if(sttlmtAcct || clrSys){
			...
			retVal = setCommentsForTransaction("00", "7830", map);	//FldTag not present
	...
	if(sttlmtMtd == 'COVE'){
		if(!instdRmbrsmntAgt || !instgRmbrsmntAgt){
			...
			retVal = setCommentsForTransaction("00", "7831", map);	//FldTag not present
	...
	if(sttlmtMtd == 'CLRG'){
		if(sttlmtAcct || instgRmbrsmntAgt || instdRmbrsmntAgt || thrdRmbrsmntAgt || instdRmbrsmntAgtAcct || instgRmbrsmntAgtAcct || thrdRmbrsmntAgtAcct){
			...
			retVal = setCommentsForTransaction("00", "7832", map);	//FldTag not present
...
function genericEitherFieldPresentRuleCamt029(Document, map){
	...
	if(crrctnTx){
		if(!instrId && !endToEndId){
			...
			retVal = setCommentsForTransaction("00","5961", map);//FldTag not present
		...
		if(!instrId && !endToEndId && !txId){
			...
			retVal = setCommentsForTransaction("00","5963", map);//FldTag not present
		...
		if(!reqdExctnDt && !reqdColltnDt){
			...
			retVal = setCommentsForTransaction("00","5962", map);//FldTag not present
...
function grpHdrDrctDbtTxInfFldCompRulePacs003(Document, map) {
	...
	logger.info("grpHdr_CdtTrfTxInf_FldCompRulePacs003 = " + pacs03ValdFlagMx);
	...
	if(pacs03ValdFlagMx == 'ERROR') {
		if(grpHdrInstgAgt && cdtrTrfinInstgAgt){
			...
			retVal = setCommentsForTransaction("138", "5702", map);
			return retVal;
		}
		if(!grpHdrInstgAgt && !cdtrTrfinInstgAgt){
			setHeader(map, "PLCN_validMessage",false);
			logger.info("grpHdrDrctDbtTxInfFldCompRulePacs003 Error block: InstgAgt must be present in grp header or in transaction.");
			retVal = setCommentsForTransaction("138", "8020", map);
			return retVal;
		}
		if(grpHdrInstdAgt && cdtrTrfinInstdAgt){
			setHeader(map, "PLCN_validMessage",false);
			logger.info("grpHdrDrctDbtTxInfFldCompRulePacs003: If GrpHdr/InstdAgt is present, then DrctDbtTxInf/InstdAgt is not allowed.");
			retVal = setCommentsForTransaction("139", "5701", map);
			return retVal;
		}
		if(!grpHdrInstdAgt && !cdtrTrfinInstdAgt){
			setHeader(map, "PLCN_validMessage",false);
			logger.info("grpHdrDrctDbtTxInfFldCompRulePacs003 Error block: InstdAgt must be present in grp header or in transaction.");
			retVal = setCommentsForTransaction("139", "8019", map);
			return retVal;
		}
		if(grpHdrPmtTpInf && cdtrTrfinPmtTpInf){
			setHeader(map, "PLCN_validMessage",false);
			...
			retVal = setCommentsForTransaction("037", "5703", map);
			return retVal;
		}
		if(!grpHdrPmtTpInf && !cdtrTrfinPmtTpInf){
			setHeader(map, "PLCN_validMessage",false);
			logger.info("grpHdrDrctDbtTxInfFldCompRulePacs003 Error block: payment type info must be present in grp header or in transaction.");
			retVal = setCommentsForTransaction("037", "8021", map);
			return retVal;
		}
	}
	if(pacs03ValdFlagMx == 'WARNING') {
		if(grpHdrInstgAgt && cdtrTrfinInstgAgt){
			setHeader(map, "PLCN_validMessage",false);
			logger.info("grpHdrDrctDbtTxInfFldCompRulePacs003: If GrpHdr/InstgAgt is present, then DrctDbtTxInf/InstgAgt is not allowed.");
			retVal = setCommentsForTransaction("138", "7040", map);
			return retVal;
		}
		if(!grpHdrInstgAgt && !cdtrTrfinInstgAgt){
			setHeader(map, "PLCN_validMessage",false);
			logger.info("grpHdrDrctDbtTxInfFldCompRulePacs003 Warning loop: InstgAgt must be present in grp header or in transaction.");
			retVal = setCommentsForTransaction("138", "8020", map);
			return retVal;
		}
		if(grpHdrInstdAgt && cdtrTrfinInstdAgt){
			setHeader(map, "PLCN_validMessage",false);
			logger.info("grpHdrDrctDbtTxInfFldCompRulePacs003: If GrpHdr/InstdAgt is present, then DrctDbtTxInf/InstdAgt is not allowed.");
			retVal = setCommentsForTransaction("139", "7039", map);
			return retVal;
		}
		if(!grpHdrInstdAgt && !cdtrTrfinInstdAgt){
			setHeader(map, "PLCN_validMessage",false);
			logger.info("grpHdrDrctDbtTxInfFldCompRulePacs003 Warning block: InstdAgt must be present in grp header or in transaction.");
			retVal = setCommentsForTransaction("139", "8019", map);
			return retVal;
		}
		if(grpHdrPmtTpInf && cdtrTrfinPmtTpInf){
...
function b2bInstAmtExchRateSepaPacs003(Document, map) {
	...
	if(instdAmtVal){
		if((instdAmtCcy == intrBkSttlmtCcy) && xchgRateVal){
			setHeader(map, "PLCN_validMessage",false);
			retVal = setCommentsForTransaction("055", "7924", map);
		...
		if((instdAmtCcy != intrBkSttlmtCcy) && !xchgRateVal){
			setHeader(map, "PLCN_validMessage",false);
			retVal = setCommentsForTransaction("055", "7923", map);
	...
	if(!instdAmtVal){		
		if(xchgRateVal){
			setHeader(map, "PLCN_validMessage",false);
			retVal = setCommentsForTransaction("055", "7907", map);
 ...
 function grpHdrTtlintrBkSttlmAmtRulePacs003(Document, map){
	...
	if(res){
		if(res != intrBkSttlmAmt){
				...
				retVal = setCommentsForTransaction("050","5707", map);
				return retVal;
			}		
	}
...
function b2bIntrBnkSttltDateSepaPacs003(Document, map) {
	...
	if(ttlIntrBkSttlmAmt){
		if(!grpHdrIntrBkSttlmDt && !cstmrDrctDbtintrBkStlDt){
				...
				retVal = setCommentsForTransaction("051", "5773", map);	
				return retVal;	 
		}
	}
	return retVal;
...
function genericMustPresentRulePacs003(Document, map) {
	...
	if(intrmyAgt1Acct){
		if(!intrmyAgt1){
			...
			retVal = setCommentsForTransaction("140", "5817", map);		
			return retVal;	
		}
	}
	...
	if(intrmyAgt2Acct){
		if(!intrmyAgt2){
			...
			retVal = setCommentsForTransaction("142", "5818", map);		
			return retVal;	
		}
	}
	...
	if(intrmyAgt2){
		if(!intrmyAgt1){
			...
			retVal = setCommentsForTransaction("140", "5814", map);		
			return retVal;	
		}
	}
	...
	if(intrmyAgt3Acct){
		if(!intrmyAgt3){
			...
			retVal = setCommentsForTransaction("144", "5819", map);		
			return retVal;	
		}
	}
	...
	if(intrmyAgt3){
		if(!intrmyAgt2){
			...
			retVal = setCommentsForTransaction("142", "5815", map);		
			return retVal;	
		}
	}
	...
	if(isPatternPresent(documentString, "<Dbtr>")){
		...
		if(isPatternPresent(documentString, "<UltmtDbtr>")){
			...
			if(ultmtDbtrNm == dbtrNm || ultmtDbtrPstlAdr == dbtrPstlAdr || ultmtDbtrId == dbtrId || ultmtDbtrCtryOfRes == dbtrCtryOfRes || ultmtDbtrCtctDtls == dbtrCtctDtl){
				...
				setHeader(map, "PLCN_validMessage", false);
				retVal = setCommentsForTransaction("173", "5745", map); //NEW violations to be defined..
				return retVal;
			}
		}
	...
	if(isPatternPresent(documentString, "<Cdtr>")){
		...
		if(isPatternPresent(documentString, "<UltmtCdtr>")){
			...
			if(ultmtCdtrNm == cdtrNm || ultmtCdtrPstlAdr == cdtrPstlAdr || ultmtCdtrId == cdtrId || ultmtCdtrCtryOfRes == cdtrCtryOfRes || ultmtCdtrCtctDtls == cdtrCtctDtl){
				logger.info("cdtrUltmtCdtrContentCheckPacs003: UltimateCreditor may only be present if different from Creditor.");
				setHeader(map, "PLCN_validMessage", false);
...
function eitherFieldPresentRulePacs003(Document, map){
	...
	if(!txId && !uetr){
		...
		setHeader(map,"PLCN_validMessage",false);
		logger.info("eitherFieldPresentRulePacs003 : TxId or UETR must be present, Both may be present.");
		retVal = setCommentsForTransaction("031","7138", map);
		return retVal;
	}
	return retVal;
}
...
function AmdmntIndSepaPacs007Rule(Document,map){
	...
	if(amdmntInd == "FALSE" && amdmntInfDtls){
		...
		retVal = setCommentsForTransaction("076", "7937", map);	
		return retVal;
		...
		retVal = setCommentsForTransaction("076", "7936", map);	
		return retVal;		
	}
	return retVal;
}
...
function chargesInfoSepaPacs007Rule(Document,map){
	...
	if(chrgsInf && !rvsdInstdAmt){
		setHeader(map, "PLCN_validMessage",false);
		logger.info("chargesInfoSepaPacs007Rule: If ChrgsInf is present, then RvsdInstdAmt must be present.");
		retVal = setCommentsForTransaction("049", "7139", map);
		return retVal;
	}
	return retVal;
}
...
function OrgnlGrpInfoSepaPacs007Rule(Document,map){
	...
	if(orgnlGrpInf && orgnlGrpInf2){
		...
		retVal = setCommentsForTransaction("038", "7054", map);	
		return retVal;
	}
	return retVal;
}
...
function sttlmMtdRuleSepaPacs007(Document, map) {
	...
	if(sttlmMtdValue == "INDA" || sttlmMtdValue == "INGA"){
		if(thrdRmbrsmntAgtCheck || instgRmbrsmntAgtCheck || instdRmbrsmntAgtCheck || clrSysCheck){
			setHeader(map, "PLCN_validMessage",false);
			retVal = setCommentsForTransaction("013", "7829", map);
	...
	if(sttlmMtdValue == "COVE"){
		if(sttlmAcctCheck || clrSysCheck){
			setHeader(map, "PLCN_validMessage",false);
			retVal = setCommentsForTransaction("013", "7830", map);
	...
	if(sttlmMtdValue == "COVE"){
		if(!instgRmbrsmntAgtCheck && !instdRmbrsmntAgtCheck){
			setHeader(map, "PLCN_validMessage",false);
			retVal = setCommentsForTransaction("013", "7831", map);
	...
	if(sttlmMtdValue == "CLRG"){
		if(thrdRmbrsmntAgtCheck || instgRmbrsmntAgtCheck || instdRmbrsmntAgtCheck || sttlmAcctCheck){
			setHeader(map, "PLCN_validMessage",false);
			retVal = setCommentsForTransaction("013", "7832", map);
...
function genericMustPresentRulePacs007(Document, map) {
	...
	if(instgRmbrsmntAgtAcct){
		if(!instgRmbrsmntAgt){
			...
			retVal = setCommentsForTransaction("018", "7834", map);		
			return retVal;	
		}
	}
	...
	if(instdRmbrsmntAgtAcct){
		if(!instdRmbrsmntAgt){
			...
			retVal = setCommentsForTransaction("020", "7835", map);		
			return retVal;	
		}
	}
	...
	if(thrdRmbrsmntAgtAcct){
		if(!thrdRmbrsmntAgt){
			...
			retVal = setCommentsForTransaction("022", "7833", map);		
			return retVal;	
		}
	}
	...
	if(thrdRmbrsmntAgt){
		if(!instgRmbrsmntAgt && !instdRmbrsmntAgt){
			...
			retVal = setCommentsForTransaction("012", "7828", map);		
			return retVal;	
		}
	}
	return retVal;
...
function intrBnkSttltDateSepaPacs007(Document, map) {
	...
	if(ttlIntrBkSttlmAmt){
		if(!grpHdrIntrBkSttlmDt){
				setHeader(map, "PLCN_validMessage",false);
				logger.info("intrBnkSttltDateSepaPacs007:If TtlRvsdIntrBkSttlmAmt is present, then IntrBkSttlmDt must be present.");
				retVal = setCommentsForTransaction("011", "7140", map);	
				return retVal;	 
		}
	}
	return retVal;
