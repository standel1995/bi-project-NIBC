...
function calculateHash(exchange) {
	...
	logger.info("calculateHash: orgnlBody = " + orgnlBody);
	inMsg.setBody(orgnlBody);*/

	result = getHeader(map, "PLCN_duplicateMessage");
	logger.trace("calculateHash: PLCN_duplicateMessage = " + result);
	...
	logger.trace("calculateHash:PLCN_duplicateMessage in calculatehash = " + getHeader(map, "PLCN_duplicateMessage"));
}
...
function calculateBusiness(exchange) {
	...
	logger.info("businessDuplicateMx: duplicateCheck = " + duplicateCheck);
	flag =  memTblGetTableValue(map, "INST_PARAM",duplicateCheck);
	logger.info("businessDuplicateMx: flag value = " + flag);
	...
	inMsg.setBody(orgnlBody);
	result = getHeader(map, "PLCN_duplicateMessage");
	logger.trace("calculateBusiness: PLCN_duplicateMessage = " + result);
...
function duplicateCheckCorpMX(exchange) {
	...
	custom5Dupl = "PREV=" + parentMsgDbId + "|ADOR" + "¿";
	...
	if(msgId && parentMsgDbId) {
		if(parentMsgDbId < msgId) {
			...
			custom5Dupl = "PREV=" + orgMsgId + "|ADOR" + "¿"; //PREV=25539|ADOR?
...
function businessDuplicateMx(exchange) {
	...
	executeRoute.callRouteWithHeader('direct://businessCheck', body, hdrMap);
  	...
  	var outHdrMap = executeRoute.getOutputHeader();
  	var outmsg = executeRoute.getOutputBody(java.util.List.class);
  	logger.info("businessDuplicateMx: Output Body = " + outmsg );
...
function duplicateSelectQueryMX(exchange) {
	...
	custom5DuplPrev = getHeader(map, "custom5Dupl"); //PREV=25539|ADOR?
	...
	if(queueId == "MXDUPLQ") {
		...
		if(custom5DuplPrev) {
			custom5Dupl = custom5DuplPrev.concat(custom5Dupl); //PREV=25539|ADOR?
		}
		else{
			custom5Dupl = "";
		}
		setHeader(map, "custom5DuplPrev" , custom5Dupl); //PREV=25539|ADOR?
